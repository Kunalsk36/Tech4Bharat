"use client";

import { useEffect } from "react";

/* ===============================
   ICONS (UNCHANGED)
================================ */

export const NationalInitiativeIcon = () => (
  <svg
    viewBox="0 0 24 24"
    width="24"
    height="24"
    fill="none"
    stroke="currentColor"
    strokeWidth="1.8"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    <circle cx="12" cy="12" r="9" />
    <path d="M3 12h18" />
    <path d="M12 3a15 15 0 0 1 0 18" />
    <path d="M12 3a15 15 0 0 0 0 18" />
  </svg>
);

export const MultidisciplinaryIcon = () => (
  <svg
    viewBox="0 0 24 24"
    width="24"
    height="24"
    fill="none"
    stroke="currentColor"
    strokeWidth="1.8"
    strokeLinecap="round"
    strokeLinejoin="round"
  >
    <circle cx="6" cy="6" r="2" />
    <circle cx="18" cy="6" r="2" />
    <circle cx="12" cy="18" r="2" />
    <path d="M8 6h8M7 8l4 8M17 8l-4 8" />
  </svg>
);

/* ===============================
   COMPONENT
================================ */

export default function AboutTechForBharat() {
  useEffect(() => {
    const elements = document.querySelectorAll(".reveal");
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target.classList.add("active");
            observer.unobserve(entry.target);
          }
        });
      },
      { threshold: 0.15 }
    );

    elements.forEach((el) => observer.observe(el));
    return () => observer.disconnect();
  }, []);

  return (
    <section className="relative py-15 sm:py-15 reveal">
      <div
        className="
          max-w-6xl mx-auto
          bg-gradient-to-b
          from-[#edeef8]
          to-[#c5d1ff]
          backdrop-blur-lg
          rounded-3xl shadow-xl
          px-6 sm:px-10 lg:px-12 py-16
        "
      >
        {/* HEADER */}
        <div className="text-center mb-15">
          <h2
            className="
              text-3xl sm:text-4xl font-bold
              bg-gradient-to-r
              from-[#020024]
              via-[#090979]
              to-[#00D4FF]
              bg-clip-text text-transparent
            "
          >
            About Tech For Bharat
          </h2>
          <p className="mt-3 text-gray-600 text-base sm:text-lg">
            Empowering India’s youth with cutting-edge technology skill
          </p>
        </div>

        {/* WHO WE ARE */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-15">
          {/* LEFT CARD */}
          <div className="bg-[#173b6c] text-white rounded-2xl p-8 shadow-lg card-glow">
            <h3 className="text-2xl font-semibold mb-4">Who We Are</h3>
            <p className="text-[15px] leading-relaxed mb-4 opacity-90">
              Tech For Bharat is a national initiative focused on building a
              future-ready generation.
            </p>
            <p className="text-[15px] leading-relaxed opacity-90">
              We work at the intersection of technology, governance, and
              innovation.
            </p>
          </div>

          {/* RIGHT INFO CARDS */}
          <div className="lg:col-span-2 space-y-6">
            {[
              {
                title: "National Initiative",
                desc: "Driving awareness and capability-building through a technology-first vision.",
                icon: <NationalInitiativeIcon />,
              },
              {
                title: "Multi-disciplinary Learning",
                desc: "Blending technology, policy, security, and innovation.",
                icon: <MultidisciplinaryIcon />,
              },
            ].map((item, i) => (
              <div
                key={i}
                className="
                  flex items-start gap-4
                  bg-white rounded-xl p-6
                  shadow-md card-glow
                "
              >
                <div
                  className="
                    w-12 h-12 rounded-full
                    bg-[#4f6ff2]/15
                    flex items-center justify-center
                    text-[#4f6ff2]
                    shrink-0
                  "
                >
                  {item.icon}
                </div>

                <div>
                  <h4 className="text-lg font-semibold text-[#020024]">
                    {item.title}
                  </h4>
                  <p className="text-[15px] text-gray-600">
                    {item.desc}
                  </p>
                </div>
              </div>
            ))}
          </div>
        </div>

        {/* WHAT WE DO */}
        <div className="mb-15">
          <h3 className="text-2xl font-semibold mb-10 text-[#020024]">
            What We Do
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 ">
            {[
              "Emerging Technologies",
              "AI & Machine Learning",
              "Cyber & Digital Defence",
              "National Security Technologies",
              "Tech Governance & Policy",
              "Strategic Innovation",
            ].map((title, i) => (
              <div
                key={i}
                className="
                  border-2 border-[#173b6c]
                  rounded-xl
                  p-6 text-center
                  bg-white
                  shadow-sm card-glow
                "
              >
                <p className="text-[15px] font-medium text-[#020024]">
                  {title}
                </p>
              </div>
            ))}
          </div>
        </div>

        {/* KEY FOCUS & APPROACH */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          {/* KEY FOCUS */}
          <div>
            <h3 className="text-2xl font-semibold mb-6 text-[#020024]">
              Key Focus Areas
            </h3>

            <div className="grid grid-cols-2 gap-4">
              {[
                { code: "AI", label: "Artificial Intelligence", dark: true },
                { code: "CT", label: "Cyber Technologies", dark: true },
                { code: "TP", label: "Tech Policy", dark: false },
                { code: "EI", label: "Equity & Inclusion", dark: false },
              ].map((item, i) => (
                <div
                  key={i}
                  className={`rounded-xl p-5 text-center shadow-sm card-glow ${
                    item.dark
                      ? "bg-[#173b6c] text-white"
                      : "bg-white text-gray-700"
                  }`}
                >
                  <div className="text-lg font-bold">{item.code}</div>
                  <p className="text-[15px] mt-1 opacity-90">
                    {item.label}
                  </p>
                </div>
              ))}
            </div>
          </div>

          {/* OUR APPROACH */}
          <div>
            <h3 className="text-2xl font-semibold mb-6 text-[#020024]">
              Our Approach
            </h3>

            <ul className="space-y-4">
              {[
                "Comprehensive Learning Platform",
                "Industry & Academic Collaboration",
                "Future-Ready Skill Development",
                "Innovation & Policy Thinking",
              ].map((item, i) => (
                <li
                  key={i}
                  className="
                    flex items-center gap-4
                    bg-white rounded-full
                    px-5 py-3 shadow-sm card-glow
                  "
                >
                  <span
                    className="
                      w-7 h-7 rounded-full
                      bg-[#4f6ff2]
                      text-white
                      flex items-center justify-center
                      text-sm font-semibold
                    "
                  >
                    {i + 1}
                  </span>
                  <p className="text-[15px] text-gray-700">{item}</p>
                </li>
              ))}
            </ul>
          </div>
        </div>
      </div>
    </section>
  );
}
